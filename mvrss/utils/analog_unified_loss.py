...
class AnalogSymmetricUnifiedFocalLoss(nn.Module):
    ...
    def __init__(self, weight=0.5, delta=0.6, gamma=0.5, global_weight = 1.):
        super(AnalogSymmetricUnifiedFocalLoss, self).__init__()
        self.weight = weight
        self.delta = delta
        self.gamma = gamma
    ...
    def forward(self, y_pred, y_true):
      ...
      y_pred =  torch.stack((y_pred[:,0,:,:],torch.sum(y_pred[:,1:,:,:],dim=1)), dim=1)
...
class AnalogAsymmetricUnifiedFocalLoss(nn.Module):
    """The Unified Focal loss is a new compound loss function that unifies Dice-based and cross entropy-based loss functions into a single framework.
    Parameters
    ----------
    weight : float, optional
    ...
    def __init__(self, weight=0.5, delta=0.6, gamma=0.2, global_weight = 1.):
        super(AnalogAsymmetricUnifiedFocalLoss, self).__init__()
        self.weight = weight
        self.delta = delta
        self.global_weight = global_weight
        self.gamma = gamma
    ...
    #   y_pred = one_hot(torch.clamp(torch.argmax(y_pred,dim=1),min=0,max=1)).permute(0,-1,1,2)
      ...
      if self.weight is not None:
        return ((self.weight * asymmetric_ftl) + ((1-self.weight) * asymmetric_fl))*self.global_weight
      else:
        return (asymmetric_ftl + asymmetric_fl)*self.global_weight
